function [psth, edges, whole, frRaw] = calPSTH(trials, windowPSTH, binSize, step)
...
switch class(trials)
    case "cell"
        ...
        frRaw = cell2mat(cellfun(@(x) mHist(x, edges, binSize), trials, "UniformOutput", false)') * 1000 / binSize;
        temp = cat(1, trials{:});
        nTrials = length(trials);
        psth = mHist(temp, edges, binSize) / (binSize / 1000) / nTrials; % Hz
    case "struct"
        temp = arrayfun(@(x) x.spike(:), trials, "UniformOutput", false);
        psth = mHist(cat(1, temp{:}), edges, binSize) / (binSize / 1000) / length(trials); % Hz
        frRaw = cell2mat(cellfun(@(x) mHist(x, edges, binSize), temp, "UniformOutput", false)') * 1000 / binSize;
    case "double"

        if isvector(trials)
            psth = mHist(trials, edges, binSize) / (binSize / 1000);
        else
